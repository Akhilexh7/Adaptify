...
const AnimatedText: React.FC<{ text: string; className?: string; by?: 'letter' | 'word' }> = ({ text, className, by = 'letter' }) => {
  const items = by === 'letter' ? text.split('') : text.split(' ');

  const containerVariants = {
    hidden: {},
    visible: {
      transition: { staggerChildren: by === 'letter' ? 0.03 : 0.06 },
    },
  };
...
function MiniNavbar() {
  ...
  const navLinksData = [
    { label: 'Features', href: '#1' },
    { label: 'Analytics', href: '#2' },
    { label: 'About', href: '#3' },
  ];

  const loginButtonElement = (
    <button className="px-5 py-2 sm:px-4 text-xs sm:text-sm border border-[#333] bg-[rgba(31,31,31,0.62)] text-gray-300 rounded-full hover:border-white/50 hover:text-white transition-colors duration-200 w-full sm:w-auto">
      Sign In
    </button>
  );
  ...
  const signupButtonElement = (
    <div className="relative group w-full sm:w-auto">
       ...
       <button className="relative z-10 px-5 py-2 sm:px-4 text-xs sm:text-sm font-semibold text-black bg-gradient-to-br from-gray-100 to-gray-300 rounded-full hover:from-gray-200 hover:to-gray-400 transition-all duration-200 w-full sm:w-auto">
         Get Started
       </button>
    </div>
  );
  ...
  return (
    <header className={`fixed top-6 left-1/2 transform -translate-x-1/2 z-20
                        ...
                        w-[calc(100%-2rem)] sm:max-w-[600px]
      ...
      <div className="flex items-center justify-between w-full gap-x-6">
        ...
        <nav className="hidden sm:flex items-center justify-center flex-1 px-4">
        ...
        <div className="hidden sm:flex items-center flex-shrink-0 gap-2 sm:gap-3">
          {loginButtonElement}
          {signupButtonElement}
        </div>
        ...
        if (isComplete) {
          ...
          setTimeout(() => {
            if (typeof window !== "undefined") {
              localStorage.removeItem("__dev_logged_out__");
                ...
                {step === "email" ? (
                  ...
                  >
                    <div className="space-y-1">
                       ...
                       <p className="text-[1.25rem] text-white/50 font-light">Let's build better habits</p>
                     </div>
